...
@section('metatags')
<title>View Evaluation | OAMPI Evaluation System</title>
<style type="text/css">
a.back-to-top {
  display: none;
  width: 60px;
  ...
  <!-- Content Header (Page header) -->
    <section class="content-header">
      <h1>
        ...
        @if (count($canDoThis)>0)<a href="#" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#myModal_editEvalPeriod"><i class="fa fa-pencil"></i> Modify Eval Period</a>
        @endif
      </h1>
     ...
     <section class="content">
          ...
          <div class="row">
                          ...
                          <div class="box box-widget widget-user-2">
                            ...
                            <div class="widget-user-header  @if ($doneEval[$employee->id]['evaluated'] && ($employee->userType_id == '3') ) bg-gray @elseif ($doneEval[$employee->id]['evaluated'] && ($employee->userType_id != '3') ) bg-darkgreen @elseif (!$doneEval[$employee->id]['evaluated'] && ($employee->userType_id != '3') )bg-green  @else bg-black @endif">
                              ...
                              <div class="widget-user-image">
                                ...
                                <img src="{{asset('public/img/employees/'.$employee->id.'.jpg')}}" class="img-circle pull-left" alt="User Image" width="70" style="margin-right:5px">
                                @else
                                  <img src="{{asset('public/img/useravatar.png')}}" class="user-image pull-left" alt="Employee Image" width="70" style="margin-right:5px">
                              ...
                              <h5 class="widget-user-desc">{{$employee->position->name}}<br/>
                                @else
                              <h5 class="widget-user-desc">{{$showPosition}}<br/>
                              @endif
            ...
            <div class="col-lg-8 col-sm-7 col-xs-12">
              <div class="box box-primary">
                      ...
                      <div class="box-body">
                        ...
                        <div class="row">
                          <div class="col-lg-8">
                            @if (!$evalForm->coachingDone && Auth::user()->id == $employee->id )  <!-- only the owner of the evaluation can do the review -->
                            <a class="btn btn-sm btn-danger pull-left" href="{{action('EvalFormController@review',$evalForm->id)}} " style="margin-right:5px">
                              <i class="fa fa-search"></i> Review </a>

                            @endif



                             <a href="{{action('EvalFormController@printEval', $evalForm->id)}}" target="_blank" class="btn btn-sm btn-default pull-left">
                              <i class="fa fa-print"></i> Print PDF </a>

                             <!-- if Regularization form -->
                              @if( $evalForm->evalSetting_id == 3 && Auth::user()->userType_id !== 4 && !$evalForm->isDraft && ($updateStatus == 'true' ) ) 
                              <a href="{{action('MovementController@createNew',['id'=>$evalForm->user_id, 'evaluatedBy'=>$evalForm->evaluatedBy])}} " target="_blank" class="btn btn-sm btn-danger pull-left" style="margin-left:10px">
                              <i class="fa fa-user"></i> Update Employee Status  </a>
                              @elseif ( $evalForm->evalSetting_id == 3 && Auth::user()->userType_id !== 4 && !$evalForm->isDraft ) 
                              <a href="{{action('MovementController@createNew',$evalForm->user_id)}} " target="_blank" class="btn btn-sm btn-danger pull-left" style="margin-left:10px">
                              <i class="fa fa-user"></i> Update Employee Status  </a>

                              @endif

                              @if ($evalForm->isDraft) <h3 class="text-center" style="color:#dd4b39; padding-left:30px">* Saved as Draft *<h3> @endif




                              <div class="clearfix"></div> <!-- onclick="window.print();return false;"  -->
                        ...
                        @if (Auth::user()->userType_id !== 4 && $allowed && !$evalForm->coachingDone)
                        <div class="clearfix"></div>
                        <a href="{{action('EvalFormController@edit', $evalForm->id)}} " id="submitBtn" class="btn btn-sm btn-primary pull-right" style="margin-bottom:20px">
                          <i class="fa fa-pencil"></i> Modify Evaluation</a>
                        @endif
                                                        ...
                                                        <table class="table">
                                                              <tr>
                                                                <td style="width:55%"><?php echo $entry['definitions'] ?>
                                                                    ...
                                                                    <textarea class="form-control" disabled="disabled">{{$formEntries[$cnt+1]['value']}} </textarea><br/><br/>
            ...
            @include('layouts.modals-editEval', [
                    'modelRoute'=>'resource.store',
                    'modelID' => '_editEvalPeriod', 
                    'modelName'=>"New OAMPI Resource file ", 
                    'modalTitle'=>'Upload', 
                    'modalMessage'=>'Evaluation Period:', 
                    'formID'=>'editEval',
...
<script>
  $(function () {
   ...
   //  e.preventDefault();
   //  alert('Modify functionality currently out of order. \n\nClick "Print PDF" button to download a backup copy of this eval and then request HR to delete this evaluation so you can re-submit a new one. \n\nSorry of the inconvenience.');
   //  return false;
 ...
 <?php $idxctr = 1; ?>
   ...
   @if( $idxctr == count($ratingScale))
    ...
    //if (rating.toPrecision(4) <= (100-((100-{{$rs->maxScore}})*0.5)) && rating.toPrecision(4) > (100-((100-{{$rs->percentage}})*0.5))-10 ){
      ...
      @if($evalForm->evalSetting_id !== 3 && $evalForm->evalSetting_id !== 4)
