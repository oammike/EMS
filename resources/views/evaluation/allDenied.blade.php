                ...
                <!-- Custom Tabs -->
                        <div class="nav-tabs-custom">
                          <ul class="nav nav-tabs">
                            ...
                            <li><a href="{{action('EvalFormController@allPendings',['type'=>6])}}">
                              <strong  class="text-default">Pending for Approval<br/> <small>evaluations</small></strong></a> </li>

                            <li ><a href="{{action('EvalFormController@allApproved',['type'=>6])}}">
                              <strong  class="text-default">Approved <br/> <small>evaluations</small> <span style="font-size:smaller" class="text-danger"></span></strong></a></li>

                            <li @if($type==6) class="active" @endif><a href="{{action('EvalFormController@allDenied',['type'=>6])}}">
                               <strong  @if($type==6) class="text-danger" @else class="text-default" @endif><i class="fa fa-times"></i> Rejected<br/> <small>evaluations</small></strong></a> </li>
  ...
  $(function () {
   ...
   @if($type==6) //2020 annual
      $("#teams").DataTable({
                    ...
                    //   $('#ct1').html(allItems);
                    // },
                   "ajax": "{{ action('EvalFormController@getAllDeniedEvals',['type'=>$type])}}",
                    ...
                    "columns": [
                        ...
                        { title: "Period Covered" ,defaultContent: "<i>empty</i>", data:'type',width:'190', render:function(data,type,full,meta){
                        ...
                        { title: "Score " ,defaultContent: "<i>empty</i>", data:'overAllScore',render:function(data,type,full,meta){
                          if(full.overAllScore <= 69.99)
                            ...
                            var increase = "8%";
                          else if (full.overAllScore > 97.40 && full.overAllScore <= 100.0)
                            var increase = "10%"
                        ...
                        modalcode += '<div class="modal fade" id="myModal2'+full.id+'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">';
                          ...
                          modalcode += '    </div>';
                          modalcode += '    <div class="modal-body">Are you sure you want to mark this evaluation as APPROVED &amp; FINAL for employee discussion?';
                          modalcode += '<br/><br/><label><input type="checkbox" name="finaleval" class="finaleval" data-evalid="'+full.id+ '" checked="checked"  value="1" /> Use as the Final Eval</label><br/><small><em>* for those with multiple evaluations due to movements</em></small><br/>';
                               
                          modalcode += '    </div>';
                          modalcode += '    <div class="modal-footer no-border">';
                          modalcode += '      <form action="'+approveLink+'" method="POST" class="btn-outline pull-right" id="approveReq"><button type="submit" class="btn btn-primary "><i class="fa fa-check"></i> Yes</button><button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button><input type="hidden" name="_token" value="'+_token+'" /><input type="hidden" name="makeFinal" value="1" /> </form>';
