...
class NewPA_Form_User_Controller extends Controller
{
    protected $user;
    protected $newPA_form_user;
    use Traits\EvaluationTraits;
    use Traits\UserTraits;

     public function __construct(NewPA_Form_User $newPA_form_user)
    {
        $this->middleware('auth');
        $this->user =  User::find(Auth::user()->id);
        $this->newPA_form_user = $newPA_form_user;
    }


	public function destroy($id)
    {
        $this->newPA_form_user->destroy($id);
        return back();
