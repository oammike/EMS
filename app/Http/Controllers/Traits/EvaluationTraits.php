                                ...
                                $hisTL = ImmediateHead::find(Team::find($hisTeam->id)->leader->immediateHead_id);
                                            ...
                                            */
                                            ...
                                            if ( $foundEval )
                                            {
                                                ...
                                                if ($theeval->isDraft) 
                                                  $doneMovedEvals[$ctr] = ['evaluated'=>1,'user_id'=>$emp->user_id, 'isDraft'=>1, 'evalForm_id'=> $evaluated->first()->id, 'score'=>$truegrade, 'startPeriod'=>$theeval->startPeriod, 'endPeriod'=>$theeval->endPeriod,'theEval'=>$theeval];
                                                else
                                                //$doneEval[$emp->id] = ['evaluated'=>1, 'evalForm_id'=> $existing->first()->id, 'score'=>$truegrade, 'startPeriod'=>$currentPeriod->format('M d, Y'), 'endPeriod'=>$endPeriod->format('M d, Y')];
