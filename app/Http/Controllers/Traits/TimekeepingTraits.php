    ...
    switch ($type) 
    {
      ...
      case 'FL':
      {
        $leaves = DB::table('user_familyleaves')->where([ 
                  ...
                  leftJoin('team','team.user_id','=','users.id')->
                  leftJoin('campaign','campaign.id','=','team.campaign_id')->select('user_familyleaves.id as leaveID', 'user_familyleaves.leaveStart','user_familyleaves.leaveEnd','user_familyleaves.isApproved','user_familyleaves.totalCredits','user_familyleaves.halfdayFrom','user_familyleaves.halfdayTo', 'user_familyleaves.created_at', 'user_familyleaves.notes','users.employeeCode as accesscode','users.nickname',  'users.id as userID','users.lastname','users.firstname','campaign.name as program','user_familyleaves.leaveType as FLtype', 'campaign.id as programID')->where('user_familyleaves.leaveStart','<=',$endCutoff->format('Y-m-d')." 23:59:00")->orderBy('user_familyleaves.isApproved','ASC')->get();
