...
class HomeController extends Controller
{
    ...
    public function index()
    {
      ...
      if (count($loggedIn) > 0) $alreadyLoggedIN=true; else $alreadyLoggedIN=false;
                ...
                $top3 = new Collection;
                $idolIDs = [ 1585, 40, 2277, 3175, 2328, 531,3112, 2708, 3027, 3260, 2723];
                $top3s = [829,674,1686];
      ...
      switch ($action) {
        ...
        case '5':{
                    if($this->user->id !== 564 ) {
                      $user = User::find(Input::get('id'));
                      $file = fopen('public/build/changes.txt', 'a') or die("Unable to open logs");
                        fwrite($file, "-------------------\n Played IntlTranslation Video by [". $this->user->id."] ".$this->user->lastname." on ". $correct->format('M d h:i A').  "\n");
                        fclose($file);
                    } 
