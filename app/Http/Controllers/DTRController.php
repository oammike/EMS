...
class DTRController extends Controller
{
    ...
    public function downloadDTRsheet(Request $request)
    {
      ...
      if($request->reportType == 'dailyLogs') // used for Finance Report
      {
        ...
        // stat_id 18 == TRAINEE_PASSED
        $passedTrainees =  DB::table('movement')->where('movement.personnelChange_id',3)->where('movement.isDone',1)->
                      where('movement.effectivity','<=',$cutoffEnd->format('Y-m-d H:i:s'))->
        ...
        Excel::create($fileType.$pname."_".$cutoffStart->format('M-d'),function($excel) use($fileType,$reportType, $program,$pname, $allDTR, $allDTRs,$ecqStats, $cutoffStart, $cutoffEnd, $headers,$description, $passedTrainees,$allUsers,$allFixedWS,$allMonthlyWS) 
               {
                      ...
                      if($reportType == 'dailyLogs') //finance summary report format
                      {
                        
                        $excel->setTitle($cutoffStart->format('Y-m-d').' to '. $cutoffEnd->format('Y-m-d').'_'.$program->name.' DTR Summary Report');
                        ...
                        $excel->sheet($payday->format('M d')."_".substr($payday->format('l'), 0,3), function($sheet) use ($program, $allDTR, $allDTRs, $ecqStats, $cutoffStart, $cutoffEnd, $headers,$payday, $passedTrainees)
                          {
                            ...
                            foreach($allDTR as $employeeDTR)
                            {
                              ...
                              if (count($dData) > 0)
                              {
                                ...
                                {
                                    $includeDate=1;
                                    // we introduce new check: kung may movement at newly hired, 
                                    // check muna kung pasok sa cutoff yung movement nya from Trainee to Passed
                                    $mayMvt = collect($passedTrainees)->where('id',$key->id);
                                    if(count($mayMvt) > 0)
                                    {
                                      $hire=Carbon::parse($key->dateHired,'Asia/Manila')->format('Y-m-d');
                                      if( $key->productionDate <= Carbon::parse($mayMvt->first()->effectivity,'Asia/Manila')->format('Y-m-d') || ($hire > $key->productionDate)  ) 
                                        $includeDate=0;
                                      else if($hire == '1970-01-01')  $includeDate=0;
