...
class UserOBTController extends Controller
{
    ...
    public function checkExisting(Request $request)
    {
        ...
        $mayExisting = User_OBT::where('user_id',$request->user_id)->where('leaveEnd','>',$vf->format('Y-m-d H:i:s'))->get();
    ...
    public function getCredits(Request $request)
    {
        ...
        $mayExisting = User_OBT::where('user_id',$user->id)->where('leaveEnd','>',$vf->format('Y-m-d H:i:s'))->get();
        $interval = new \DateInterval("P1D");
        foreach ($mayExisting as $key) {
                $period = new \DatePeriod(new \DateTime(Carbon::parse($key->leaveStart,'Asia/Manila')->format('Y-m-d')),$interval, new \DateTime(Carbon::parse($key->leaveEnd,'Asia/Manila')->addDays(1)->format('Y-m-d')));
                //** we need to add 1 more day kasi di incuded sa loop ung leaveEnd
        ...
        mayExistingReturn:
        return response()->json(['hasOBTalready'=>$hasVLalready,'existingVL'=>$coll, 'creditsToEarn'=>0, 'forLWOP'=>0, 'creditsleft'=>0, 'credits'=> 0 , 'shift_from'=>$shift_from, 'shift_to'=>$shift_to,'displayShift'=>$displayShift,  'schedForTheDay'=>null]);
