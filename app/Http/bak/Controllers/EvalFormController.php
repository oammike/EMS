...
class EvalFormController extends Controller
{
    ...
    {
        $allForms = EvalForm::where('overAllScore','!=','0')->orderBy('created_at','DESC')->take(50)->get();
        $users = User::all();
    ...
    public function edit($id)
    {
      //check first if the one editing is the evaluator
        ...
        if ($this->user->employeeNumber == $evaluator->employeeNumber)
        {
            ...
            foreach ($details as $detail) {
                    ...
                    // --------------- generate form elements based on leader/agent competencies
                          ...
                          if( $comp['acrossTheBoard'] ){
                            ...
                            if ( empty($isLead) || $employee->userType_id==4 || $employee->leadOverride  ){ //agent
                            ...
                            if ( empty($comp['percentage']) && (empty($isLead) || $employee->userType_id==4 || $employee->leadOverride ) ){ //agent sya
                              $formEntries->push([
                                'competency'=> $comp['name'], 
                                'definitions'=>$comp['definitions'],
                                'percentage'=>$comp['agentPercentage'], 
                            ...
                            } else if (!empty($comp['percentage']) && (!empty($isLead) && $employee->userType_id!=4 ) && empty($employee->leadOverride) ){ //leader sya
                             $formEntries->push([
                              'competency'=> $comp['name'], 
                              'definitions'=>$comp['definitions'],
                              'percentage'=>$comp['percentage'], 
           ...
           //return $formEntries;
           return view('evaluation.edit-employee', compact('performanceSummary','startPeriod', 'endPeriod', 'evalType', 'employee', 'ratingScale', 'evalForm','evalSetting', 'formEntries','maxScore','summaries', 'details','showPosition'));
